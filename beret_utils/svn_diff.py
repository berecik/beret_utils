...
def getCmd( src, rev_begin, rev_end ):
    if src[:7] == 'http://' :
        url = src
        cmd = "svn diff --summarize --xml -r %(rbegin)s:%(rend)s %(url)s" % {'url':url, 'rbegin': str( rev_begin ), 'rend': str( rev_end )}
    else:
...
def crateDestByUrl( url, dest, rev_end, urls ):
    ...
    for src_url in urls:
        ...
        if not os.path.exists( desc_dir ):
            os.makedirs( desc_dir )
        cmd = "svn export --force -q -r %(rend)s %(src_url)s %(desc_file)s" % { 'src_url':src_url, 'desc_file':desc_file, 'rend': rev_end}
